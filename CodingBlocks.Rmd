...
If any packages require installation through `BiocManager`, run `install.packages("BiocManager")`

Alternatively, an installation code chunk has been provided below to help install the latest versions of all of the above programs. Run this only if you need to install all programs from scratch.

```{r, echo = TRUE, results = 'hide'}
install.packages(c("BiocManager","Seurat","SCINA","ggplot2"))
BiocManager::install(c("SingleCellExperiment","scater","dplyr",
                       "scmap","celldex","SingleR","harmony",
                       "cerebroApp","msigdb"))
```
...
the index we will use to map our unlabeled data to. First, we select genes to use, which will be those deemed most informative by scmap:
...
The query dataset we will be using is provided by 10X genomics. First, using the command line, navigate to the directory that you are currently in. If you are unsure of what directory that is, run the following command:

```{r}
getwd()
```

Navigate to this directory, and the 10X genomics data can be retrieved by writing the following on the command line.
...
An alternative method is to copy and paste the same link into your browser, and the data will automatically download and can subsequently be extracted. Just make sure it ends up in the relative file path of ./filtered_gene_bc_matrices/hg19/
...
DimPlot(merged_seur, reduction="umap") + ggplot2::ggtitle("Before Integration")
...
# Plot the data
DimPlot(merged_seur, reduction="umap") + ggplot2::ggtitle("After Integration")
```
...
We can manually label the clusters based on this table or we could create a rule to algorithmically label the clusters based on this table. Since there are only 10 clusters, we assign the labels manually.
...
top5 <- markers_seur %>% dplyr::group_by(cluster) %>%
  dplyr::top_n(n = 5, wt = avg_logFC)
# Create the dot plot
DotPlot(query_seur, features = unique(top5$gene))
# Create the heatmap
DoHeatmap(query_seur, features = unique(top5$gene)) + NoLegend()
```
