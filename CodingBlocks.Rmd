...
R version | Packages                    |                |
----------|-----------------------------|----------------|-----------------
4.0.3     |SingleCellExperiment_1.11.7* |dplyr_1.0.2*    |ggplot2_3.3.2
...
Alternatively, an installation code chunk has been provided below to help install the latest versions of all of the above programs. Run this only if you need to install all programs from scratch.  Select compile from source if prompted.

```{r, echo = TRUE, results = 'hide'}
install.packages(c("BiocManager","Seurat","SCINA","ggplot2","devtools"))
BiocManager::install(c("SingleCellExperiment","scater","dplyr",
                       "scmap","celldex","SingleR","cerebroApp","msigdb"))
...
untar("pbmc3k_filtered_gene_bc_matrices.tar.gz",compressed="gzip")
```
